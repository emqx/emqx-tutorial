# EMQ X 到 Mosquitto 的桥接
EMQ X 节点可以桥接到其他类型的 MQTT 消息中间件，实现跨平台的消息订阅和发送。在这个章节我们以一个配置例来说明如何配置 EMQ X 到 Mosquitto 的桥接。

## 场景描述
假设我们有一台 EMQ X 服务器'emqx1'，和一台 Mosquitto 服务器，我们需要在'emqx1'上创建一条桥接把所有"传感器(sensor)"主题消息转发至 Mosquitto 服务器，并订阅所有"控制(control)"主题。
...
```
在 mosquitto 上对主题'control/device1/restart'发布消息：
```
