...
EMQ X 企业版 3.4.0 起提供了 Schema Registry 功能，提供编解码能力。Schema Registry 管理编解码使用的 Schema、处理编码或解码请求并返回结果。Schema Registry 配合规则引擎，可适配各种场景的设备接入和规则设计。
...
上图展示了 Schema Registry 的一个应用案例。多个设备上报不同格式的数据，经过 Schema Registry 解码之后，变为统一的 JSON 格式，然后转发给后台应用。

EMQ X 3.4.0 内置的 Schema Registry 数据格式包括 [Avro](https://avro.apache.org) 和 [Protobuf](https://developers.google.com/protocol-buffers/)。Avro 和 Protobuf 是依赖 Schema 的数据格式，编码后的数据为二进制，解码后为 JSON。解码后的数据可直接被规则引擎和其他插件使用。此外 Schema Registry 支持用户自定义的 (3rd-party) 编解码服务，通过 HTTP 或 TCP 回调的方式，进行更加贴近业务需求的编解码。
...
EMQ X 的 PUB/SUB 系统将消息路由到指定的主题。规则引擎可以灵活地配置数据的业务规则，按规则匹配消息，然后指定相应动作。数据格式转换发生在规则匹配的过程之前，先将数据转换为可参与规则匹配的 JSON 格式，然后进行匹配。
...
其他配置保持默认。emqx 会分配一个 Schema ID 和 Version。如果是第一次创建名为 my_parser 的编解码，Schema ID 为 "my_parser:1.0"。
...
```sql
SELECT
  schema_encode('my_parser:1.0', payload) as encoded_data,
  schema_decode('my_parser:1.0', encoded_data) as decoded_data
FROM
  "message.publish"
WHERE
  topic =~ 't/#'
...
- `schema_encode` 函数将 payload 字段的内容按照 'my_parser:1.0' 这个 Schema 来做编码，结果存储到 `encoded_data` 这个变量里;
- `schema_decode` 函数将 payload 字段的内容按照 'my_parser:1.0' 这个 Schema 来做解码，结果存储到 `decoded_data` 这个变量里;
...
Selected Data 里面的两个字段 `decoded_data` 和 `decoded_data` 对应 SELECT 语句里面的两个 AS。因为 `decoded_data` 是编码然后再解码之后的结果，所以它又被还原为了我们发送的内容 "hello"，表明编解码插件工作正常。
