...
EMQ X可以限定特定客户可以使用的主题。如果要做这样的限定，需要在EMQ X配置中启用ACL（Access Control List），并禁用匿名访问和关闭无ACL命中的访问许可（为了测试调试方便，在默认配置中，后两项是开启的，请注意关闭）。
...
共享订阅是MQTT5标准的新特性，在标准发布前，EMQ X就已经把共享订阅作为标准外特性进行了支持。在普通订阅中，所有订阅者都会收到订阅主题的所有消息，而在共享订阅中，订阅同一个主题的客户端会轮流的收到这个主题下的消息，也就是说同一个消息不会发送到多个订阅者。
...
**什么是代理订阅？使用场景是什么？**
通常情况客户端需要在连接到EMQ X之后主动订阅主题。代理订阅是指broker为客户端订阅主题，这一过程不需要客户端参与，客户端和需要代理订阅的主题的对应关系保存在Broker侧。
...
在精心设计和实现之后，单个EMQ X的节点就可以处理百万级的连接。
...
EMQ X 企业版支持消息持久化，可以将消息保存到数据库。

目前EMQ X企业版消息持久化支持的数据库有：
- Redis；
- MongoDB；
- MySQL；
- PostgreSQL；
...
能。目前EMQ X企业版支持把消息桥接至Kafka进行流式处理。

**EMQ X支持集群吗？有哪些实现方式？**

EMQ X支持集群。集群可以通过手动配置或自动配置的方式实现。
